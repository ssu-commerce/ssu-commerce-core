package com.ssu.commerce.core.security.filter
...
class JwtAuthenticationFilter(
    ...
    override fun doFilter(request: ServletRequest, response: ServletResponse, filterChain: FilterChain) {
        try {
            val (accessToken: String?, claims: Claims?) = jwtTokenProvider.resolveToken(request as HttpServletRequest)
            if (accessToken !== null && claims !== null)
                getContext().authentication = jwtTokenProvider.getAuthentication(accessToken, claims)
